...
```
Since there are 32 variables in the new data set, we calculated CH(K) (Calinski-Harabasz criterion) and W(K) (total within-cluster sum of squares) at different k ranging from k=2 to k=32 while holding the numebr of centers to be 50. As a result, we found that the maximum CH at k=3 and the minimum W at k at 32. However, if were to build a kmeans clustering model based on either k=3 or k=32, would we be able to find any useful pattern among each cluster in order to better understand NutrientH20's social-media audience? In the following steps, we tried to answer this question by subjectively evaluating categories contained within each cluster.
...
kmeans_sm8$size
```
One can see from the output at each of step, as k increases from 2 to 6, categories in each cluster start to show patterns which can be picked up by business intuition. When k=5 and k=6, we can almost associate each clsuter to a specific customer group in the business context. However, as k continues to grow from 6 and on, we see more catergories overlappig among clusters and patterns start to disappear. For model simplicity, we believe the best kmeans model is at k=5. But can we we optimize the model by minimizing the total within-cluster sum of squares? To answer this, we tried kmeans++ which initializes the cluster centers before proceeding with the standard kmeans. This time, we only tested kmeans++ with k=2 to k=8. 
...
plot(w ~ k,type='b',co="red")
par(new=TRUE)
plot(wpp ~ k,type='b',co="red")

```
Similar to patterns identified in kmeans approach, we found the most meaningful kmeans++ model to be the one with k=5. Compared to the kmeans model with k=5, the total within-cluster sum of squares is reduced. Believing, k=5 is the most practical number of clusters, we further tried out a few hierarchical clustering models with different linkage methods.
...
```
We tried hierarchical clustering models for k=5 for min, max and average methods and found out they do not produce meaningful business results in this case because most of the data points lie under cluster 1 (see the Cluster Dendrograms ablove). Finally, kemans++ with k=5 was chosen as our best model. We visualized the name of each category within each cluster ordered and colored by its frequency in the step below:
...
####Here is a summary showing our understanding of each cluster identified through our best model:
...
####Cluster3: These twitter followers are most likely to be young colleage students who really care about appreance and enjoy                    playing sports and online games.
...
####The followings are our marketing strategy recommendations:
