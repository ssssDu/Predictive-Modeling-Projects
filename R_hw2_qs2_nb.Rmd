...
We defined a function called readerPlain to readin the content of text files.
...
By using the for loop here, we got the author name for each txt files in the training data set and also the file path toward each text files for each authors in the training set.
We then combined all the text files in the training set to make it into corpus by using Corpus() function
...
For testing set, we repeated what we did for the training set and make a corpus for the testing data.
...
After we removed numbers, punctuations, excess white-space and stopwords, we got our new corpus for the training set.
...
By using class functions, we finally got our sparse matrix for the training and testing sets respectively. In addition, inspect function were used here to access values inside of the sparse matrix.
...
We then went on removing some sparse terms in our sparse matrix for the training and testing sets. After that, we converted our sparse matrices to word frequency matrices for the training and testing sets in data frame type.
...
Now we tried to intersect common columns that present in both training and testing sets.
...
We used the new training data set that we got from the previous section to perform the randomforest model with tree numbers  equal to 100.
...
We used 4 different values(50,100,150,200) to figure out the best tree number for random forest model and finally came out with our highest prediction with ntree=100 and a corresponding accuracy around 60%.
