...
for ( i in 2:dim(sm_new)[2]){
  list[i-1]=kmeans(sm_new, i , 
                   nstart = 50)$betweenss/ kmeans(sm_new, i , 
                                                              nstart = 50)$tot.withinss *(dim(sm_new)[1]-i)/(i-1)
  list2[i-1]=kmeans(sm_new, i , 
...
Compared to the kmeans model with k=5, the total within-cluster sum of squares is reduced. Believing, k=5 is the most practical number of clusters, we further tried out a few hierarchical clustering models with different linkage methods.
...
Finally, kemans++ with k=5 was chosen as our best model. We visualized the name of each category within each cluster ordered and colored by its frequency in the step below:
